...
function Delete({ ulId, reload }: { ulId: Number, reload: Function }) {
    ...
    const handleDelete = async (ulId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/admin/api/ul/${ulId}`)
        reload()
        setIsLoading(false)
        Swal.fire({
            position: 'top-end',
        ...
        <div>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(ulId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </div>
    )
